...
describe('User routes', () => {
  describe('POST /v1/users', () => {
    ...
    test('should return 201 and successfully create new user if data is ok', async () => {
      ...
      expect(dbUser).toMatchObject({ givenName: newUser.givenName, email: newUser.email });
    });

    test('should be able to create an admin as well', async () => {
      await insertUsers([admin]);
      newUser.roles = ['5ebac534954b54139806c112'];
      ...
      const dbUser = await User.findById(res.body.id);
      expect(dbUser.roles.length).toEqual(1);
    });
  ...
  describe('GET /v1/users', () => {
    ...
    test('should correctly apply filter on name field', async () => {
      ...
      const res = await request(app)
        .get('/api/v1/users')
        .set('Authorization', `Bearer ${adminAccessToken}`)
        .query({ givenName: userOne.givenName })
        .send()
        .expect(httpStatus.OK);
  ...
  describe('PATCH /v1/users/:userId', () => {
    test('should return 200 and successfully update user if data is ok', async () => {
      ...
      expect(res.body).not.toHaveProperty('password');
      expect(res.body.givenName).toEqual(updateBody.givenName);

      const dbUser = await User.findById(userOne._id);
      expect(dbUser).toBeDefined();
      expect(dbUser.password).not.toBe(updateBody.password);
      expect(dbUser.givenName).toEqual(updateBody.givenName);
    });

    test('should return 401 error if access token is missing', async () => {
      await insertUsers([userOne]);
      const updateBody = { givenName: faker.name.findName() };
